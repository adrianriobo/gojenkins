...
IMG ?= quay.io/ariobolo/gojenkins:v${VERSION}
...
$(BUILD_DIR)/gojenkins: $(SOURCES)
	GOOS=linux GOARCH=amd64 go build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/gojenkins $(GO_EXTRA_BUILDFLAGS) ./cmd

.PHONY: build 
build: $(BUILD_DIR)/gojenkins
